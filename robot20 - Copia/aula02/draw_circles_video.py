...
while(True):
    ...
    if circles is not None:        
        ...
        for i in circles[0,:]:
            ...
            cor_menor = np.array([160,  50,  50])
            cor_maior = np.array([170, 255, 255])
            mask_magenta = cv2.inRange(circles, cor_menor, cor_maior)

            cor_menor = np.array([93, 50, 50])
            cor_maior = np.array([103, 255, 255])
            mask_azul = cv2.inRange(circles, cor_menor, cor_maior)

            


            ## Somar 2 filtros --> cv2.sum
            # identificar cada um
