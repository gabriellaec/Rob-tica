...
while(True):
    ...
    circles = None
    circles=cv2.HoughCircles(bordas,cv2.HOUGH_GRADIENT,2,40,param1=50,param2=100,minRadius=5,maxRadius=50)


    # Fazedor de retas
    reta = None
    reta = cv2.HoughLinesP(bordas,1,np.pi/180,100,0,100)
    
    magenta_menor = np.array([170,  50,  50])
    magenta_maior = np.array([175, 255, 255])
    mask_coke_mag = cv2.inRange(hsv, magenta_menor, magenta_maior)

    ciano_menor = np.array([106, 50, 50])
    ...
    #reta do centro
    cv2.line(bordas_color,(0,240),(640,240),(255,0,0),3)
